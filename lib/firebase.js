...
const firebaseConfig = {
  ...
  // Optional — only used if Analytics is added later. We do NOT init analytics
  // here (it needs browser-only guards), so this is harmless when unset.
...
// ---- User profile doc at users/{uid} (separate from the leads subcollection) ----
// Best-effort: profile writes never block auth. If Firestore rules don't yet
// allow the users/{uid} doc, sign-in still succeeds and leads still work.

// Create/merge the profile on account creation.
export async function createUserProfile(uid, email) {
  ...
  try {
    await setDoc(
      ...
      { merge: true } // never overwrite future profile fields
    );
  } catch { /* profile is best-effort; rules may not allow it yet */ }
}

// Update login timestamps on normal sign-in (merge, so nothing else is touched).
export async function touchUserLogin(uid, email) {
  if (!db) return;
  try {
    await setDoc(
      doc(db, "users", uid),
      { uid, email: email || "", lastLoginAt: serverTimestamp(), updatedAt: serverTimestamp() },
      { merge: true }
    );
  } catch { /* best-effort */ }
}
...
// Used by the search-result merge — kept unchanged.
...
// Read every saved lead as a FULL document (all stored fields + id = leadKey).
// Used to populate the Ready to Contact / Pipeline / Archive stage tabs.
export async function fetchSavedLeadDocs(uid) {
  if (!db) return [];
  const snap = await getDocs(collection(db, "users", uid, "leads"));
  const docs = [];
