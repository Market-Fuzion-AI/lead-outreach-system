...
function attachSocial(place, socialResults, field) {
  ...
  for (const r of socialResults) {
    ...
    const rt = tokens(hay);
    const shared = rt.filter((t) => nameTokens.has(t)).length;
    if (shared >= 2) {
      place[field] = r.link || "";
      return;
    }
...
export async function searchLocalBusinesses({ niche, location, maxResults, apiKey }) {
  ...
  for (const place of places) {
    attachSocial(place, igResults, "instagram_url");
    attachSocial(place, fbResults, "facebook_url");
  }
