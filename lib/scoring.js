...
export const FALLBACK_MESSAGES = {
  first: "Quick question — do you already have a simple system for capturing new inquiries from your website, calls, and social pages?",
  followUp1: "Reason I ask is I noticed you have a few public channels where people can reach out, and a simple follow-up flow can help make sure those inquiries do not slip through.",
  followUp2: "If useful, I can show you a simple version that captures inquiries, asks a couple of qualifying questions, and points people to the right next step.",
  closeTheLoop: "Totally understand if this is not a priority right now. I'll close the loop here — just thought it may be useful since missed follow-up is one of those problems that is easy to overlook until leads disappear.",
};
...
export function fallbackBusinessRead(b = {}, s) {
  ...
  if (s.hasReviews) {
    out.push(`Public signals look ${s.reviewCount >= 200 ? "strong" : "present"} — around ${b.reviews} reviews${s.hasRating ? ` at ${b.rating}★` : ""}, which likely means steady inbound interest.`);
  }
